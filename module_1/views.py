...
def signup(request):
    ...
    if request.POST.get("btnSignUp"):
        ...
        if (
            form_signup.is_valid()
            and form_signup.cleaned_data["password"]
            == form_signup.cleaned_data["password_confirm"]
        ):
            ...
            post.password = hasher.encode(
                form_signup.cleaned_data["password"], "12345678"
            )
            post.save()
...
def create_author_and_book(request):
    ...
    return render(
        request,
        "module_1/create_author_and_book.html",
        {
            "formauthor": formauthor,
            "formbook": formbook,
        },
...
def search(request):
    ...
    return render(request, "module_1/search_results.html", {"results": results})
